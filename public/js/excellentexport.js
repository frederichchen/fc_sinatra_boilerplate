...
ExcellentExport = (function() {
    ...
    var tableToCSV = function(table, omitColNum) {
        var data = "";
        var i, j, row, col;
        for (i = 0; i < table.rows.length; i++) {
            row = table.rows[i];
            for (j = 0; j < row.cells.length-omitColNum; j++) {
                col = row.cells[j];
    ...
    var ee = {
        ...
        /** @expose */
        csv: function(anchor, table, omitColNum, delimiter, newLine) {
            ...
            table = get(table);
            var csvData = tableToCSV(table, omitColNum);
